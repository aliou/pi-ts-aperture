...
- `dedicated/api-routing.ts` - Aperture compatibility-to-Pi API mapping (`getApiForCompatibility`) and stream-time dispatch (`buildStreamSimple`) for dedicated mode. Per-model endpoint-to-API mapping lives in the shared `src/endpoint-mapping.ts` (`getApiForEndpoints`); per-API gateway base-URL resolution lives in the shared `src/base-url-routing.ts` (`getBaseUrlForApi`).
...
- `api/client.ts` - Pi-agnostic Aperture API client for `/api/providers` and `/v1/models` (connectors via `/api/connectors`). `providers()` cross-references `/v1/models` so disabled providers — whose models never appear there — are filtered out, leaving only enabled, callable providers. Each model's `supported_endpoints` and `pricing` are retained on `modelInfoById`.
...
- `endpoint-mapping.ts` - Shared Aperture endpoint-to-Pi-API mapping. `getApiForEndpoints` maps `/v1/models` `supported_endpoints` to a Pi `Api`, returning `null` for absent/empty/unrecognized lists so the caller falls back to the provider-level `compatibility` map.
...
- Registers the Pi provider with a custom `aperture` API and routes each request through the target Pi API selected from per-model `supported_endpoints` (when reported by `/v1/models`) or the provider-level `compatibility` map.
...
- Fetches provider compatibility from `/api/providers` (each gateway provider reports its `compatibility` map) and maps it to Pi APIs: OpenAI chat/completions, Anthropic messages, OpenAI responses, Gemini generate content, Google Vertex, or Bedrock converse. Per-model `supported_endpoints` from `/v1/models` takes precedence for OpenAI/Anthropic/responses APIs when present; the `compatibility` map is the fallback for older gateways and for Gemini/Vertex/Bedrock (whose endpoints are not exposed via `supported_endpoints` on current gateways).
