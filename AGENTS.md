...
- Proxy provider selection maps Aperture providers to local Pi registry providers by base URL (admin-only, from `/aperture/config`), including child path matching. If base URLs are unavailable (e.g. non-admin grants that get 403 on `/aperture/config`), it falls back to exact provider IDs from `/api/providers`. A persisted `apertureProviderId` override is honored on auto-matched providers so hand-edited overrides are not dropped.
...
- Optional per-provider gateway model verification (`shouldCheckGatewayModels`) warns if configured local models are missing from the Aperture gateway. The check looks models up under the entry's `apertureProviderId` (defaulting to `id`), so it works for manual mappings where the Pi provider id differs from the Aperture gateway provider id; the warning labels the mapping as `piId -> apertureId` when they differ.
- Removed proxy providers trigger unregistration.
- Manual Pi→Aperture provider mapping: `proxy.upstreamProviders[].apertureProviderId` (optional, defaults to `id`) names the Aperture gateway provider a Pi provider maps to. `mapProxyProviders` MERGES automatic matches with persisted entries so manual mappings survive a settings submenu reopen instead of being clobbered. This is the escape hatch for non-admin grants that get 403 on `/aperture/config` and so cannot use automatic base-URL matching: they add a mapping via the settings Proxy submenu or the onboarding proxy step and it persists. Manual entries that no longer match a local Pi provider are kept but flagged stale (not silently deleted). The settings Proxy submenu and the onboarding proxy step share a custom `ProxyUpstreamProvidersEditor` (`extensions/aperture/settings/proxy-upstream-editor.ts`) that lists EVERY local Pi provider as a row (including ones that did not auto-match, e.g. `anthropic`/`openai-codex` when the gateway uses different provider ids), shows `[auto]`/`[manual]` origin tags, and reuses `FilterableChecklist` (search input + list) for both the provider list and the Aperture target picker so the user can type to filter. Actions use keys that never collide with typing: `Space` set/change the Aperture target, `Ctrl+G` (per-row, via `FilterableChecklist.onCtrlG` now receiving the highlighted item id) toggle the gateway model check, and clearing a mapping is done by picking the `— Clear mapping —` row in the picker. In settings `Esc` backs out; in onboarding `Enter` continues (list mode) / backs out of the picker, because the wizard intercepts `Esc`. An auto-matched provider can be re-pointed at a different Aperture gateway endpoint, and a provider with no auto-match can still be mapped. There is no separate `enabled` flag in the config yet, so disabling a mapping removes it from `upstreamProviders`; auto-matched providers are re-derived on the next open, but a disabled manual mapping is lost and must be re-added. (Persisting disabled mappings is a known follow-up.)
- Routing never uses `apertureProviderId`: Aperture routes by the model id in the request body, and proxy registration only overrides `baseUrl`/`apiKey`/headers on the existing Pi provider. `apertureProviderId` is used only for the gateway model check and display.
- A `004` migration backfills `apertureProviderId = id` on existing entries (disk normalization); the loader's `afterMerge` (`normalizeProxiedProviders`) is the runtime safety net that fills `apertureProviderId ?? id` and `shouldCheckGatewayModels ?? true` so the resolved `Required` shape holds even for hand-edited configs.
