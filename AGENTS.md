...
- Config-branch limits, all of them consequences of merge-by-id. Each model keeps its own `api` (a provider can span APIs), and its base URL is resolved from that api, matching the native branch. A **passthrough** provider is skipped with a warning: its credential cannot be expressed in a config, and emitting one with `models` and no `apiKey` is rejected by the host and aborts the whole sync. **`keepGatewayModelsOnly`** can only choose what to reroute, never what to hide — the models it leaves out keep their upstream definitions — so it warns rather than pretending to filter. A registration the host refuses is caught per provider so one bad provider does not cost the rest of the list.
