...
- Proxy and dedicated modes share one gateway base-URL resolver, `getBaseUrlForApi` in `src/base-url-routing.ts`. Anthropic and Codex map to the gateway root (Pi's SDKs append their own paths); Gemini to `/v1beta`; Vertex to `/v1`; Bedrock to `/bedrock`. OpenAI SDK APIs use `gateway/v1` by default, with `openaiRoute: "root"` available for deployments exposing `/chat/completions` and `/responses` directly at the gateway origin; the existing upstream version-segment rule still routes providers such as Z.ai to the root when needed.
...
- Per-model base URL is inferred from the upstream provider's base URL, looked up from Pi's native model registry (cross-referenced by provider id, then model id). Both modes resolve the per-model base URL with the shared `getBaseUrlForApi`: OpenAI SDK APIs use `gateway/v1` by default and honor the top-level `openaiRoute` override (`"root"` for gateways exposing root OpenAI routes), while providers whose upstream base URL ends in a non-`/v1` version segment (e.g. Z.ai `/api/coding/paas/v4`) still use the gateway root. Anthropic, Gemini, Vertex, and Bedrock keep their fixed paths (`/bedrock` for Bedrock). Gateway providers with no native Pi registry match keep the configured OpenAI route.
