...
- `base-url-routing.ts` - Shared gateway-root-vs-`gateway/v1` inference (`shouldUseGatewayRootForProxy`) from the Pi API and the upstream provider base URL. Used by both proxy and dedicated modes.
